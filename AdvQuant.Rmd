...
#Garrett's new code 2.0 with Priscilla's notes from class - Create an intermediate file 
```{r}
...
Intermediate <- Spring_Data_raw %>% 
  filter(is.na(Date == TRUE)) %>% #<= means filter for instances where Date is NA because NA is recognized as a completely empty cell in numeric/date columns
...
Spring_Data_raw %>% fill(Transect)
```

#Data Sleuthing - Hits
```{R}
Spring_Data_raw$Hits %>% unique ()
Spring_Data_raw %>% filter(is.na(Hits) == TRUE)
